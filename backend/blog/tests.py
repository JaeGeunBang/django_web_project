...
class TestView(TestCase):
    ...
    def navbar_test(self, soup):
        ...
        logo_btn = navbar.find('a', text='Django Test')
    ...
    def test_post_detail(self):
        ...
        # 2.1 첫 번째 post url로 접근하면 정상적으로 작동한다. (state code: 200)
        response = self.client.get(self.post_001.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')

        # 2.2 포스트 목록 페이지와 같은 네비게이션이 있다.
        self.navbar_test(soup)
    ...
    def test_create_post(self):
        # 로그인을 안한다면, 200이 나오면 안됨
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)

        # staff 가 아닌 trump가 로그인할 경우
        self.client.login(username='trump', password='somepassword')
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)

        # staff 인 obama로 로그인할 경우
    ...
    def test_delete_comment(self):
        ...
        self.assertEqual(Comment.objects.count(), 1)
        self.assertEqual(self.post_001.comment_set_count(), 1)